...
export default function PlacePage() {
    ...
    return (
        <div className="mx-auto max-w-screen-lg">
                ...
                <div className="relative">
                    <div className="grid gap-2 grid-cols-[2fr_1fr] rounded-3xl overflow-hidden">
                        <div>
                            {place.photos?.[0] && (
                                <div className="">
                                    <img onClick={() => {setShowAllPhotos(true)}} className="aspect-square cursor-pointer object-cover" src={`http://localhost:4000/uploads/${place.photos[0].filename}`} alt="" />
                                </div>
                            )}
                        </div>
                        <div className="grid">
                            {place.photos?.[1] && (
                                <div className="">
                                    <img onClick={() => {setShowAllPhotos(true)}} className="aspect-square cursor-pointer object-cover" src={`http://localhost:4000/uploads/${place.photos[1].filename}`} alt="" />
                                </div>
                            )}
                            <div className="overflow-hidden">
                                {place.photos?.[2] && (
                                    <div className="">
                                        <img onClick={() => {setShowAllPhotos(true)}} className="aspect-square cursor-pointer object-cover relative top-2" src={`http://localhost:4000/uploads/${place.photos[2].filename}`} alt="" />
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            <div className="bg-white -mx-8 px-8 py-8 border border-t border-gray-100 shadow">
                <div>
